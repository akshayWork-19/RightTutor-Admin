...
import { RootState, setUser, logout, setRetention, runCleanup, addLog } from '../store';
        ...
        <div className="space-y-8">
          ...
          <div className="ds-card p-8 space-y-6">
             ...
             <div className="space-y-4">
                ...
                <button 
                  onClick={() => { dispatch(runCleanup()); alert('Cleanup task executed successfully.'); }}
             ...
             <p className="text-[10px] text-[var(--text-muted)] italic">Self-cleaning task removes logs and temporary sheet entries older than the selected timeframe.</p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="ds-card p-12">
            ...
            <div className="flex items-center justify-between pt-10 border-t border-[var(--border-ds)]">
              <button 
                ...
                className={`ds-button ${
                  saveStatus === 'saved' ? 'bg-emerald-500 text-white' : 'bg-neutral-900 text-white hover:bg-neutral-800 dark:bg-[var(--bg-input)] dark:hover:bg-[var(--bg-hover)] dark:border dark:border-[var(--border-ds)]'
                }`}
              >
